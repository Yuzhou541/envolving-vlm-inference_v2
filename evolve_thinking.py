"""Simple AlphaEvolve-style search loop for Qwen3-VL-2B-Thinking inference code."""
...
TARGET_MODEL_NAME = "Qwen/Qwen3-VL-2B-Thinking"
MUTATOR_MODEL_NAME = "Qwen/Qwen3-VL-2B-Instruct"
DEFAULT_RUNTIME_WEIGHT = 0.05

MUTATION_FOCI = [
    "reduce answer-tag leakage and suppress visible reasoning",
...
def make_mutation_prompt(
    ...
    return textwrap.dedent(
        ...
        - Keep the target model as Qwen/Qwen3-VL-2B-Thinking.
...
def _legacy_heuristic_mutation(parent_code: str, rng: random.Random, focus: str, error_profile: dict) -> tuple[str, str]:
    ...
    if "prefer Not Applicable over guessing from nearby text" not in parent_code:
        add_option(
            "\"If the requested element does not exist, output <answer>Not Applicable</answer>.\\n\"\n",
            "\"If the requested element does not exist, output <answer>Not Applicable</answer>.\\n\"\n"
            "    \"If the chart element is missing or unclear, output <answer>Not Applicable</answer> instead of guessing from nearby text.\\n\"\n",
    ...
    add_option(
        ...
        "\"Do not output any text before or after the <answer> tags.\\n\"\n"
        "    \"Do not output <think>, <final_answer>, markdown fences, or explanations.\\n\"\n",
        "Strengthened the answer-tag rule to suppress leaked reasoning and extra wrapper text.",
        ("reduce answer-tag leakage and suppress visible reasoning", "speed"),
    )
    add_option(
...
def run_evolution(args):
    ...
    run_name = args.run_name or f"thinking_search_{timestamp}"
...
def main():
    parser = argparse.ArgumentParser(description="Run a simple local evolution loop for manual Thinking inference code.")
    parser.add_argument("--seed-module", default="manual_thinking")
    ...
    parser.add_argument("--candidates-per-generation", type=int, default=4)
    ...
    parser.add_argument("--export-best", default="evolved_thinking.py")
